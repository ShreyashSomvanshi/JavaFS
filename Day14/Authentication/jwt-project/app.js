...
// //Dummy iusers 
// const users = [
//    {id:1, username:'admin' , password:'$2a$10$vMKde3Ruz/PGX2cOJLvB7OccT9QgR2BBKFYve0Efnj6PIEbOdheau'} ,
//    {id:2, username:'user1' , password:'$2a$10$X4HrpDfR79hXPDB6g2OyNO3W8AP2VM5ifkYkec3M2b6H.p92tcSq2'} 
// ];
// let employees =[
...
// middleware to check if the user is an admin
const isAdmin = (req, res, next) => {
    if(req.user.username !== 'admin') return res.status(403).send('Access denied,Admin only.');
    next();
};
...
// app.post('/userdata', authenticate, async (req, res)=>{
//     const {username, password} = req.body;
//     const user = users.find((user) => user.username === username);
//     if(!user) return res.status(401).send('Invalid username or password');
//     const isMatch = await bcrypt.compare(password, user.password);
...
app.delete('/removeemployee/:id', authenticate, isAdmin, (req, res)=>{
    const empId = parseInt(req.params.id);
    const empIndex = users.findIndex(e=>e.id === empId);
    if(empIndex === -1) return res.status(404).send('Employee not found');
...
// route to display user data (requires jwt auth)
app.get('/userdata', authenticate, (req, res)=>{
    const user = req.user;
    res.json({id:user.id, username:user.username, position:user.position});
});
...
app.post('/adduser', authenticate, isAdmin, async (req, res)=>{
    ...
    const newUser = {id:user.length+1, username, password:hashedPassword, position};
    users.push(newUser);
    res.status(201).send('User added!');
});
